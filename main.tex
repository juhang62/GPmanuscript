...
Third, it has built-in quantification of uncertainty and is easy to implement. The advantage of using GP is that posterior variance is readily available so that uncertainty in its prediction is naturally quantified. The ease of propagating uncertainty is due to our choice of the squared exponential covariance function for which integration can be done analytically (see the Appendix). Hence the propagation of uncertainty is simply done by repeatedly applying the formula (\ref{eq:E(u)}) and (\ref{eq:cov(u)}), in contrast to ensemble methods and Bayesian methods, which reply on computer simulations \citep{lipkova2019personalized,Kostelich2011a}.   
